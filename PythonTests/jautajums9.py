...
from jautajums10 import Jautajums_10

Window.size = (550, 550)
Window.clearcolor = (110/255, 55/255, 110/255, 1)
Window.title = "Python tests - 9. jautājums"

class Jautajums_9(App):
    ...
    def __init__(self, punkti, **kwargs):
        ...
        self.question = Label(
            text='Kuri no sekojošajiem operatoriem ir piešķiršanas operatori?',
            font_size='20sp',
            halign='center',
            valign='middle',
            size_hint=(1, 0.5)
    ...
    def submit_answer(self, instance):
        is_correct = self.checkbox2.active and self.checkbox4.active and not self.checkbox3.active and not self.checkbox1.active
        
        if is_correct and self.first_attempt:
            self.punkti += 1
            self.stop()
            Jautajums_10(punkti=self.punkti).run()
        elif is_correct:
            self.stop()
            Jautajums_10(punkti=self.punkti).run()
        else:
