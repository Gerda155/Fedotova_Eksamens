...
class Jautajums_1(App):
    ...
    def __init__(self, punkti, **kwargs):
        ...
        self.question = Label(
            text='Izvēlies sadalīšanas operācijas.',
            font_size='20sp',
            halign='center',
            valign='middle',
            size_hint=(1, 0.5)
    ...
    def build(self):
        ...
        checkbox_spacer = Widget(size_hint_y=None, height=5)
        ...
        mid_spacer = Widget(size_hint_y=None, height=15)
    ...
    def submit_answer(self, instance):
        ...
        if is_correct and self.first_attempt:
            self.punkti += 1
            self.stop()
            Jautajums_2(punkti=self.punkti).run()
        elif is_correct:
            self.stop()
            Jautajums_2(punkti=self.punkti).run()
        else:
