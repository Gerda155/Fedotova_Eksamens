...
from jautajums7 import Jautajums_7

Window.size = (550, 550)
Window.clearcolor = (110/255, 55/255, 110/255, 1)
Window.title = "Python tests - 6. jautājums"

class Jautajums_6(App):
    ...
    def __init__(self, punkti, **kwargs):
        ...
        self.question = Label(
            text='Kuri no sekojošajiem operatoriem tiek izmantoti loģiskai operācijai?',
            font_size='20sp',
            halign='center',
            valign='middle',
            size_hint=(1, 0.5)
        ...
        self.checkbox1, self.checkbox1_layout = self.create_checkbox('and')
        self.checkbox2, self.checkbox2_layout = self.create_checkbox('&&')
        self.checkbox3, self.checkbox3_layout = self.create_checkbox('or')
        self.checkbox4, self.checkbox4_layout = self.create_checkbox('not')
    ...
    def submit_answer(self, instance):
        is_correct = self.checkbox1.active and self.checkbox3.active and not self.checkbox2.active and not self.checkbox4.active
        
        if is_correct and self.first_attempt:
            self.punkti += 1
            self.stop()
            Jautajums_7(punkti=self.punkti).run()
        elif is_correct:
            self.stop()
            Jautajums_7(punkti=self.punkti).run()
        else:
